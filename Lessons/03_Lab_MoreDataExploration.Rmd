...
```{r DataExercise-1a}
high_t<-c(64,71,72,63,51,52,61,64,67,73)
low_t<-c(42,49,60,46,32,32,41,47,52,54)
```
...
conversion<-function(x){
  cel<-round((x-32)*(5/9),2)
  return(cel)
}

celcius_ht<-conversion(high_t)
celcius_lt<-conversion(low_t)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
high_t.df<-as.data.frame(high_t)
temperature<- cbind(high_t.df,low_t,celcius_ht,celcius_lt)
formal_temperature<-data.frame("highest_temperature_fahrenheit"=high_t,"lowest_temperature_fahrenheit" = low_t, "highest_temperature_celcius"=celcius_ht, "lowest_temperature_celcius"=celcius_lt)
```
...
#Since the format is month-day-year we will use function mdy()
date_juneteenth <- mdy(str_juneteenth)
date_juneteenth
```
